...
| 21 | New column added to a table whose name contains a non-ASCII character, left unquoted (e.g. `bücher`) — the easy, additive case routed to `internal/sqldefwrap` rather than `internal/rebuild` | Safe — direct `ADD COLUMN`; the shared `internal/sqlident.IsIdentByte` was still ASCII-only even after scenarios 19/20 fixed this locally inside `schemadiff`'s and `rebuild`'s own tokenizers, but the actual failure here was one layer upstream of that predicate: the vendored sqldef library's own SQLite DDL parser only recognizes ASCII letters/digits/underscore in an unquoted identifier and rejects `$`/non-ASCII outright with a syntax error, so `generate` failed even for this otherwise-easy additive change. `internal/sqlident.IsIdentByte` was fixed to match SQLite's actual grammar (also used to remove the now-duplicate local checks scenarios 19/20 had added), and `internal/sqldefwrap.Diff` now double-quotes any bare `$`/non-ASCII identifier in its input before handing it to sqldef's parser, since a newer sqldef release doesn't fix this (already pinned to the latest) |
| 22 | New nullable column added to a `STRICT` table whose existing columns include unquoted `serial` and `key` — `key` also used in a table-level `UNIQUE (key)` — and the new column is itself named with a colliding word (`value`) | Safe — direct `ADD COLUMN`; sqldef's shared multi-dialect grammar rejects a long list of bare words (`serial`, `key`, `value`, `text`, `date`, `time`, `json`, `uuid`, and dozens more) as column/table names that SQLite itself accepts as ordinary identifiers, so any additive change to a table naming one unquoted failed the same way scenario 21 did, at the same layer (`internal/sqldefwrap`). Unlike `$`/non-ASCII, these are ordinary ASCII words already used all over real schemas as column types, constraint keywords, or type-parameter tokens, so blanket-quoting every occurrence would corrupt sqldef's input (`text` the type, `key` inside `PRIMARY KEY`). `internal/sqldefwrap.Diff` now walks each `CREATE TABLE` statement's actual structure and quotes a bare colliding word only at the specific identifier positions it can appear in — the table name; each column-definition's own name; the column lists inside `PRIMARY KEY (…)`/`UNIQUE (…)`/`FOREIGN KEY (…)`; and a `REFERENCES` target's table/column list — leaving every other occurrence (types, inline constraint keywords, expression content) untouched. Which words need quoting is derived by probing sqldef's own SQLite parser and memoizing the answer per word, rather than a hard-coded list, so the set tracks future sqldef keyword-table changes automatically. Identifiers inside `CHECK`/`DEFAULT`/`GENERATED` expressions stay out of scope for the quoting this scenario does: scenario 24 masks a rejected expression body opaquely instead, which covers both an unsupported operator and a colliding identifier used inside one |
| 23 | New nullable column added to a `STRICT` table that already has an explicit `CREATE INDEX` (plus `AUTOINCREMENT`, several column `CHECK`s, and multiple table-level `UNIQUE`s) | Safe — direct `ADD COLUMN`, index untouched; `cmd/sqlite-migrate/generate.go`'s `additiveChangeReproducesAfter` used to replay the table's bare `CREATE TABLE` text without its indexes and compare the result to the desired table, which does carry them via `schemadiff.Table.Indexes` — the comparison could never be empty, so any additively-changed table with an explicit index silently fell back to a full rebuild. Fixed by replaying the table's target explicit (`origin = 'c'`) indexes after the added columns' `ALTER` statements before comparing, so both sides carry the same index set — including when the same edit adds or drops an explicit index (possibly on the new column itself), which `buildMigrationBody` emits separately and stays a direct `ADD COLUMN` + `CREATE`/`DROP INDEX`. Because that scratch replay's table is empty — and SQLite only rejects an `ADD COLUMN` with a non-constant `DEFAULT` (`CURRENT_TIMESTAMP`, a function call, `GLOB`/`MATCH`, …) once the table has rows — each added column's `DEFAULT` is first probed against a one-row table, and one SQLite would reject there routes the table to a rebuild. `generate` now also prints one stderr note per table this fallback still rebuilds (a real CHECK/COLLATE/GENERATED change riding along with the added column, or such a non-constant `DEFAULT`), naming the table and why, instead of choosing a rebuild silently |
| 24 | New nullable column added to a `STRICT` table whose existing `CHECK` uses a SQLite operator sqldef's shared multi-dialect grammar has no token for (`mountpoint TEXT NOT NULL CHECK (mountpoint GLOB '/mnt/disks/*')`) | Safe — direct `ADD COLUMN`; sqldef's parser also rejects `GLOB`, `MATCH` and `IS`/`IS NOT <expr>` outright, so any additive change to a table using one in a `CHECK`/`DEFAULT`/`GENERATED` expression failed the same way scenario 22 did, at the same layer (`internal/sqldefwrap`), for any *valid* SQLite operator sqldef simply lacks rather than a quoting gap. `internal/sqldefwrap.Diff` now walks each `CREATE TABLE`'s `CHECK (…)`, `DEFAULT (…)` and `[GENERATED ALWAYS] AS (…)` expression bodies (column- and table-level) and, for a body sqldef's own parser rejects (probed and memoized per exact text, the same technique scenario 22 uses for keywords), replaces it with an opaque placeholder identifier before the call and restores the original text in its output afterward — keyed by exact body text, so the same expression on both sides of a Diff call gets the same placeholder and sqldef sees no change there. A body sqldef already accepts passes through untouched, and an unresolved placeholder left in sqldef's output is refused as an error rather than passed through, since executing it would either fail outright or, worse, silently write the placeholder into a migration file. Treating each body as opaque, rather than finding identifier positions inside it, also lifts scenario 22's own "stays out of scope" limit: a colliding keyword used as an identifier inside an expression is masked the same way an unsupported operator is |
